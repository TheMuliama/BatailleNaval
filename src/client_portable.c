 ...
 * Format attendu : "L1C1L2C2...LnCn;Touches;MessageFin"
 */
void deserialiser_etat(const char *buf_in, int plateau[GRILLE_TAILLE][GRILLE_TAILLE], int *touches, char *message_fin, size_t max_msg_len) {
    ...
    // Dupliquer la chaîne pour l'analyse avec strtok, car strtok modifie son entrée.
    char temp_buf[BUFSZ];
    ...
    if (token && strlen(token) == GRILLE_TAILLE * GRILLE_TAILLE) {
        for (int l = 0; l < GRILLE_TAILLE; l++) {
            for (int c = 0; c < GRILLE_TAILLE; c++) {
                // Le serveur envoie -1, 0 ou 1 sous forme de char 'M'
                char state_char = token[l * GRILLE_TAILLE + c];
                plateau[l][c] = state_char - '0'; // Convertir le caractère chiffre en int
            }
        }
    } else {
        printf("Erreur de format de plateau.\n");
        return;
    }
    ...
    if (token) {
        *touches = atoi(token);
        ...
        printf("Erreur de format pour les touches.\n");
        return;
    }
    ...
    if (token) {
        ...
        message_fin[0] = '\0'; // Pas de message de fin
    }
}
